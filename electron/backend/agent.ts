...
import { walkthroughSchema, validateWalkthroughDocument } from '../../shared/schema.js';
import type { CommandRunner } from './github.js';

export const MAX_PROVIDER_OUTPUT = 8 * 1024 * 1024;
export const SKILL_REFERENCE_URL = 'https://raw.githubusercontent.com/warpdotdev/common-skills/main/.agents/skills/pr-walkthrough/SKILL.md';
...
const COMMON_PROVIDER_ENV_KEYS = new Set([
  ...
  'PATH', 'HOME', 'USERPROFILE', 'USER', 'USERNAME', 'LOGNAME', 'TMPDIR', 'TMP', 'TEMP', 'SHELL', 'TERM',
  'TERM_PROGRAM', 'TERM_PROGRAM_VERSION', 'COLORTERM', 'LANG', 'LANGUAGE',
  'TZ', 'NO_COLOR', 'FORCE_COLOR',
  // Platform/runtime paths and certificate configuration.
  'APPDATA', 'LOCALAPPDATA', 'PROGRAMDATA', 'SYSTEMROOT', 'WINDIR', 'COMSPEC', 'PATHEXT',
  'XDG_CONFIG_HOME', 'XDG_CONFIG_DIRS', 'XDG_CACHE_HOME', 'XDG_DATA_HOME', 'XDG_DATA_DIRS', 'XDG_STATE_HOME', 'XDG_RUNTIME_DIR',
  ...
  'HTTP_PROXY', 'HTTPS_PROXY', 'ALL_PROXY', 'NO_PROXY', 'DISABLE_TELEMETRY',
]);

const PROVIDER_AUTH_ENV_KEYS: Record<AgentProvider, ReadonlySet<string>> = {
  codex: new Set([
    'CODEX_HOME',
    'OPENAI_API_KEY', 'OPENAI_BASE_URL', 'OPENAI_API_BASE', 'OPENAI_API_TYPE', 'OPENAI_API_VERSION',
    'OPENAI_ORG_ID', 'OPENAI_ORGANIZATION', 'OPENAI_PROJECT',
    'AZURE_OPENAI_API_KEY', 'AZURE_OPENAI_ENDPOINT', 'AZURE_OPENAI_API_VERSION',
  ]),
  claude: new Set([
    'CLAUDE_CONFIG_DIR', 'CLAUDE_CODE_USE_BEDROCK', 'CLAUDE_CODE_USE_VERTEX',
    'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC', 'CLAUDE_CODE_OAUTH_TOKEN',
    'ANTHROPIC_API_KEY', 'ANTHROPIC_AUTH_TOKEN', 'ANTHROPIC_BASE_URL',
  ]),
  cursor: new Set([
    'CURSOR_API_KEY', 'CURSOR_AGENT_API_KEY', 'CURSOR_AUTH_TOKEN', 'CURSOR_API_BASE', 'CURSOR_CONFIG_DIR',
  ]),
};
...
const SECRET_ENV_NAME_PATTERN = /(?:^|[_-])(?:API[_-]?KEY|KEY|TOKEN|SECRET|PASSWORD|PASSWD|AUTHORIZATION|AUTH[_-]?TOKEN|CREDENTIALS?|PRIVATE[_-]?KEY)(?:$|[_-])/i;
...
export function buildProviderEnvironment(source: NodeJS.ProcessEnv): NodeJS.ProcessEnv;
export function buildProviderEnvironment(provider: AgentProvider, source?: NodeJS.ProcessEnv): NodeJS.ProcessEnv;
export function buildProviderEnvironment(
  providerOrSource: AgentProvider | NodeJS.ProcessEnv = process.env,
  source: NodeJS.ProcessEnv = process.env,
): NodeJS.ProcessEnv {
  const provider = typeof providerOrSource === 'string' ? providerOrSource : undefined;
  const environmentSource = typeof providerOrSource === 'string' ? source : providerOrSource;
  ...
  for (const [key, value] of Object.entries(environmentSource)) {
    ...
    if (value !== undefined && (COMMON_PROVIDER_ENV_KEYS.has(normalizedKey) || normalizedKey.startsWith('LC_') || providerKeys?.has(normalizedKey))) {
 ...
 */
export function redactProviderStderr(stderr: string, source: NodeJS.ProcessEnv = process.env): string {
  ...
  let redacted = stderr;
  for (const value of secretValues) redacted = redacted.replace(new RegExp(escapedRegExp(value), 'g'), '[REDACTED]');
  ...
  redacted = redacted.replace(/\b(?:Proxy-)?Authorization[ \t]*([=:])[ \t]*(?:Bearer|Basic|Token)[ \t]+[^\s,;}\])]+/gi, (_match, separator: string) => `Authorization${separator} [REDACTED]`);
  redacted = redacted.replace(/\bBearer[ \t]+[^\s,;}\])]+/gi, 'Bearer [REDACTED]');
  redacted = redacted.replace(/\b(Token|Secret|Password)[ \t]+[^\s,;}\])]+/gi, '$1 [REDACTED]');
...
export function redactProviderOutput(output: string, source: NodeJS.ProcessEnv = process.env): string {
  return redactProviderStderr(output, source);
}

/** Redact string leaves in a validated provider document before persistence. */
export function redactProviderDocument(value: WalkthroughDocument, source: NodeJS.ProcessEnv = process.env): WalkthroughDocument {
  const redact = (entry: unknown): unknown => {
    ...
    return Object.fromEntries(Object.entries(entry as Record<string, unknown>).map(([key, nested]) => [key, redact(nested)]));
  };
  return redact(value) as WalkthroughDocument;
}

export interface ProviderSpawn {
  (file: string, args: string[], options: { cwd: string; stdio: 'pipe'; windowsHide: boolean; env?: NodeJS.ProcessEnv }): ChildProcess;
}
...
export function buildAnalysisPrompt(request: AnalysisRequest, inputDirectory?: string): string {
  const inputLocation = inputDirectory ? ` The deterministic run inputs are available at this absolute path: ${inputDirectory}. Read those artifacts directly; do not search outside the worktree and input directory.` : '';
...
export async function detectProvider(
  ...
  try {
    const result = await runner.run(executable, ['--version'], providerCommandOptions(provider, 5_000));
    const safeStdout = redactProviderOutput(result.stdout);
    const safeStderr = result.stderr ? redactProviderStderr(result.stderr) : '';
    const version = safeStdout.trim().split(/\r?\n/)[0]?.slice(0, 200) || safeStderr.trim().split(/\r?\n/)[0]?.slice(0, 200);
    return providerStatus(provider, displayName, executable, capabilities, version);
  } catch {
    return providerStatus(provider, displayName, executable, capabilities, undefined, `${displayName} was not found or could not be started.`);
  }
}
...
function modelCandidate(value: unknown): string | undefined {
  ...
  const candidate = value.trim().replace(/^[-*]\s+/, '').replace(/\s+\([^)]*\)\s*$/, '');
  if (candidate.includes('[REDACTED]')) return undefined;
  return /^[A-Za-z0-9][A-Za-z0-9._:/\[\]=,+-]{0,199}$/.test(candidate) ? candidate : undefined;
}

function providerCommandOptions(provider: AgentProvider | undefined, timeout: number): Parameters<CommandRunner['run']>[2] {
  return { timeout, env: provider ? buildProviderEnvironment(provider) : buildProviderEnvironment() };
}
...
export function parseProviderModels(raw: string): AgentModelOption[] {
  ...
  const add = (value: unknown, label?: unknown, description?: unknown, isDefault?: unknown) => {
    const candidate = modelCandidate(value);
    if (candidate && !seen.has(candidate)) {
      seen.add(candidate);
      const display = typeof label === 'string' && label.trim() ? label.trim().slice(0, 200) : candidate;
      const detail = typeof description === 'string' && description.trim() ? description.trim().slice(0, 500) : undefined;
      models.push({ id: candidate, label: display, ...(detail ? { description: detail } : {}), ...(isDefault === true ? { isDefault: true } : {}) });
    }
  };
  const walk = (value: unknown): void => {
    ...
    let handled = false;
    if ('models' in object) { walk(object.models); handled = true; }
    if ('id' in object) { add(object.id, object.label ?? object.displayName ?? object.name, object.description, object.isDefault ?? object.default); handled = true; }
    else if ('model' in object) { add(object.model, object.label ?? object.displayName ?? object.name, object.description, object.isDefault ?? object.default); handled = true; }
    else if ('name' in object) { add(object.name, object.label ?? object.displayName, object.description, object.isDefault ?? object.default); handled = true; }
    if (!handled) Object.values(object).forEach((entry) => { if (Array.isArray(entry) || (entry && typeof entry === 'object')) walk(entry); });
  };
  const lines = raw.split(/\r?\n/);
  const hasPlainListingHeader = lines.some((line) => /^\s*Available models\s*:?\s*$/i.test(line));
  ...
  try { walk(JSON.parse(raw)); } catch { /* provider may return one model per line */ }
  for (const line of lines) {
    const trimmed = line.trim();
    if (!trimmed) continue;
    try { walk(JSON.parse(trimmed)); continue; } catch { /* plain model listing */ }
...
export async function discoverProviderModels(runner: CommandRunner, executable: string, provider?: AgentProvider): Promise<AgentModelOption[]> {
  try {
    const result = await runner.run(executable, ['models'], providerCommandOptions(provider, 10_000));
...
export function parseClaudeModelHelp(raw: string): AgentModelOption[] {
  const models: AgentModelOption[] = [];
  const seen = new Set<string>();
  const ignored = new Set(['alias', 'aliases', 'and', 'default', 'e', 'e.g', 'e.g.', 'example', 'examples', 'full', 'g', 'id', 'model', 'models', 'name', 'or', 'option', 'options', 'the', 'to', 'use']);
  const add = (value: string) => {
    const candidate = modelCandidate(value);
    if (!candidate || ignored.has(candidate.toLowerCase()) || seen.has(candidate)) return;
    seen.add(candidate);
    models.push({ id: candidate, label: candidate });
  };
  const tokens = (fragment: string) => {
    for (const match of fragment.matchAll(/[A-Za-z0-9][A-Za-z0-9._:/+=-]{1,199}/g)) add(match[0]);
  };
  const lines = raw.split(/\r?\n/);
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (const match of text.matchAll(/\(([^)]*(?:e\.g\.|examples?|aliases?|options?)[^)]*)\)/gi)) tokens(match[1]);
    for (const match of text.matchAll(/(?:e\.g\.|for example|examples?)\s*[:=-]?\s*([^).;\n]+)/gi)) tokens(match[1]);
    for (const match of text.matchAll(/[`'\"]([^`'\"]+)[`'\"]/g)) tokens(match[1]);
  }
  return models;
}
...
export function discoverCodexModels(
  ...
  return new Promise((resolve) => {
    ...
    const handleLine = (line: string) => {
      if (!line.trim()) return;
      let message: unknown;
      try { message = JSON.parse(redactProviderOutput(line, environmentSource)); } catch { return; }
      ...
      if (id === initializeId && !initializeAcknowledged) {
        ...
        finish(result === undefined ? [] : parseProviderModels(JSON.stringify(result)));
      }
    };
    ...
    try {
      child = spawn(executable, ['app-server'], { cwd: process.cwd(), stdio: 'pipe', windowsHide: true, env: environment });
    } catch {
      return finish([]);
    }
    ...
    timer.unref?.();
    write({ id: initializeId, method: 'initialize', params: { clientInfo: { name: 'pr-atlas', title: 'PR Atlas', version: '1.0.0' }, capabilities: { experimentalApi: false, requestAttestation: false } } });
  });
}

/** Discover Claude models from its CLI help when no models subcommand exists. */
export async function discoverClaudeModels(runner: CommandRunner, executable = 'claude'): Promise<AgentModelOption[]> {
  const generic = await discoverProviderModels(runner, executable, 'claude');
  if (generic.length) return generic;
  try {
    const result = await runner.run(executable, ['--help'], providerCommandOptions('claude', 10_000));
    return parseClaudeModelHelp(redactProviderOutput(`${result.stdout}\n${result.stderr ?? ''}`));
  } catch {
    return [];
  }
}
...
export async function runProviderProcess(
  ...
  if (!installation.installed) return { status: 'failed', rawOutput: '', logs: [], errors: [installation.error ?? `${adapter.displayName} is unavailable.`] };
  ...
  void runner;
  progress('inspecting', `Starting ${adapter.displayName} in a read-only mode.`);
  return new Promise((resolve) => {
    let stdout = '';
    let stderr = '';
    const environmentSource = { ...process.env };
    const providerEnvironment = buildProviderEnvironment(adapter.id, environmentSource);
    const providerOutput = () => redactProviderOutput(stdout, environmentSource);
    const providerLogs = () => stderr ? [redactProviderStderr(stderr, environmentSource)] : [];
    let finished = false;
    ...
    try {
      child = spawn(executable, args, { cwd: worktree, stdio: 'pipe', windowsHide: true, env: providerEnvironment });
    } catch {
      finish({ status: 'failed', rawOutput: '', logs: [], errors: [`${adapter.displayName} could not be started.`] });
      return;
    }
    ...
    try { child.stdin?.end(); } catch { /* process may have closed stdin */ }
    const cancel = () => {
      try { child.kill(); } catch { /* process already exited */ }
      finish({ status: 'cancelled', rawOutput: providerOutput(), logs: providerLogs() });
    };
    if (signal?.aborted) return cancel();
    signal?.addEventListener('abort', cancel, { once: true });
    child.stdout?.on('data', (chunk: Buffer | string) => { stdout = (stdout + chunk.toString()).slice(0, MAX_PROVIDER_OUTPUT); });
    child.stderr?.on('data', (chunk: Buffer | string) => { stderr = (stderr + chunk.toString()).slice(0, MAX_PROVIDER_OUTPUT); });
    child.on('error', () => finish({ status: signal?.aborted ? 'cancelled' : 'failed', rawOutput: providerOutput(), logs: providerLogs(), errors: [`${adapter.displayName} could not be started.`] }));
    child.on('close', (code) => {
      signal?.removeEventListener('abort', cancel);
      if (signal?.aborted) return finish({ status: 'cancelled', rawOutput: providerOutput(), logs: providerLogs() });
      if (code !== 0) return finish({ status: 'failed', rawOutput: providerOutput(), logs: providerLogs(), errors: [sanitizeProviderError(adapter.displayName)] });
      ...
      if (!validation.valid) return finish({ status: 'invalid', rawOutput: providerOutput(), logs: providerLogs(), errors: validation.errors });
      if (!validation.document) return finish({ status: 'invalid', rawOutput: providerOutput(), logs: providerLogs(), errors: ['Generated walkthrough was empty.'] });
      const safeDocument = redactProviderDocument(validation.document, environmentSource);
      const safeValidation = validateWalkthroughDocument(safeDocument);
      finish(safeValidation.valid
        ? { status: 'ready', document: safeDocument, rawOutput: providerOutput(), logs: providerLogs(), model: modelFromOutput(stdout, environmentSource) }
        : { status: 'invalid', rawOutput: providerOutput(), logs: providerLogs(), errors: safeValidation.errors });
    });
  });
}

/** Create a temporary Codex schema file, outside the repository worktree. */
export async function withTemporarySchema<T>(fn: (schemaPath: string) => Promise<T>): Promise<T> {
...
function unwrapOutput(value: unknown): unknown {
  ...
  const object = value as Record<string, unknown>;
  for (const key of ['structured_output', 'structuredOutput', 'result', 'output', 'output_text', 'text']) {
    if (key in object) {
      const nested = object[key];
      if (nested && typeof nested === 'object') return unwrapOutput(nested);
      if (typeof nested === 'string') {
        try { return unwrapOutput(JSON.parse(nested)); } catch { return nested; }
      }
    }
  }
  if (object.item && typeof object.item === 'object') return unwrapOutput(object.item);
  return value;
}
...
function isWalkthroughLike(value: unknown): value is Record<string, unknown> {
  ...
  const object = value as Record<string, unknown>;
  return typeof object.schemaVersion === 'string' && !!object.pullRequest && typeof object.pullRequest === 'object' && !!object.graphs && typeof object.graphs === 'object';
}

function fencedJsonCandidate(value: string): unknown {
  const matches = [...value.matchAll(/```json(?:\r?\n|\s)([\s\S]*?)```/gi)];
  if (matches.length !== 1) return undefined;
  try { return JSON.parse(matches[0][1].trim()); } catch { return undefined; }
}

function modelFromOutput(raw: string, source: NodeJS.ProcessEnv = process.env): string | undefined {
  try {
    const parsed = JSON.parse(raw) as Record<string, unknown>;
    for (const value of [parsed.model, parsed.model_name, parsed.modelName]) if (typeof value === 'string') return redactProviderOutput(value, source).slice(0, 200);
  } catch { /* output can be JSONL or plain text */ }
  return undefined;
}
...
function normalizeProviderSchema(value: unknown): unknown {
  if (Array.isArray(value)) return value.map((entry) => normalizeProviderSchema(entry));
  if (!value || typeof value !== 'object') return value;
  const source = value as Record<string, unknown>;
  const result: Record<string, unknown> = {};
  for (const [key, entry] of Object.entries(source)) {
    if (key === '$id' || key === 'additionalProperties' || key === 'required' || key === 'properties' || key === 'items') continue;
    result[key] = normalizeProviderSchema(entry);
  }
  if (source.properties && typeof source.properties === 'object' && !Array.isArray(source.properties)) {
    const properties: Record<string, unknown> = {};
    for (const [key, entry] of Object.entries(source.properties as Record<string, unknown>)) properties[key] = normalizeProviderSchema(entry);
  ...
  if (source.type === 'array') result.items = source.items === undefined ? { type: 'string' } : normalizeProviderSchema(source.items);
  return result;
}
