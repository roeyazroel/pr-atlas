...
describe("coordinator provider bootstrap", () => {
  ...
  it("boots Codex and Claude with only the task-scoped Atlas MCP and strict discovery", async () => {
    ...
    expect(claudeCalls[0].args).toEqual(expect.arrayContaining(["--safe-mode", "--setting-sources", "", "--permission-mode", "dontAsk", "--strict-mcp-config", "--mcp-config", "--settings", "--no-session-persistence", "--model", "claude-test", "--effort", "xhigh"]));
    ...
    expect(claudeTools).toBe("Read,Grep,Glob,mcp__atlas__get_task,mcp__atlas__get_anchor,mcp__atlas__get_pr_context,mcp__atlas__validate_evidence,mcp__atlas__preflight_result,mcp__atlas__report_progress,mcp__atlas__submit_result");
  ...
  it("runs Cursor from a disposable shadow worktree with its task-scoped MCP bootstrap", async () => {
    ...
    expect(calls[0].args).toEqual(expect.arrayContaining(["--workspace", "--approve-mcps", "--sandbox", "enabled", "--trust", "--force", "--model", "cursor-test"]));
    expect(calls[0].args).not.toContain("--mode");
    expect(calls[0].args).not.toEqual(expect.arrayContaining(["--yolo"]));
    expect(calls[0].args).not.toEqual(expect.arrayContaining(["--bypass-permissions"]));
    expect(calls[0].args).not.toEqual(expect.arrayContaining(["--dangerously-skip-permissions"]));
