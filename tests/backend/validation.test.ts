...
describe('analysis request validation', () => {
  ...
  it.each(['claude', 'codex', 'cursor'] as const)('accepts the %s provider', (provider) => {
  ...
  it('rejects unknown providers', () => {
    const result = validateAnalysisRequest({ ...validRequest(), provider: 'unknown' })
    expect(result.valid).toBe(false)
  ...
  it('accepts a bounded selected model and supplemental collection prompt', () => {
    const result = validateAnalysisRequest({ ...validRequest(), model: 'claude-sonnet-4-6', customPrompt: 'Collect more evidence about migrations and rollback behavior.' })
    expect(result.valid).toBe(true)
    if (result.valid) {
  ...
  it('rejects unsafe or oversized model and supplemental prompt values', () => {
    expect(validateAnalysisRequest({ ...validRequest(), model: '--dangerously-skip-permissions' }).valid).toBe(false)
    expect(validateAnalysisRequest({ ...validRequest(), customPrompt: 'x'.repeat(4_001) }).valid).toBe(false)
  })
})
