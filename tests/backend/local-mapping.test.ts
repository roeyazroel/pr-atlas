...
describe("removed local repository mapping", () => {
  it("has no renderer, IPC, contract, or analysis-service mapping path", async () => {
    for (const file of sourceFiles) {
      ...
      expect(source, file).not.toMatch(
        /mapLocalRepository|map-local-repository|Map existing checkout/,
      );
    }
  });
  ...
  it("does not keep the obsolete mapping persistence module", async () => {
    await expect(
      readFile("electron/backend/mappings.ts", "utf8"),
  ...
  it("prepares fresh analyses with a GitHub clone and an application-managed worktree", async () => {
    ...
    try {
      ...
      const service = new AnalysisService(root, { run }, undefined, undefined, [
        adapter,
      ]);
      ...
      const worktree = resolve(
        root,
        "worktrees/github.com/example/backend",
        headSha,
      );
      ...
      expect(calls).toContainEqual(
        expect.objectContaining({
          file: "gh",
          args: ["repo", "clone", "example/backend", clone],
        }),
      );
      expect(calls).toContainEqual(
        expect.objectContaining({
          ...
          args: [
            ...
            "--no-tags",
            "origin",
            "pull/42/head:refs/pr-atlas/42",
          ],
          options: expect.objectContaining({ cwd: clone }),
        }),
      );
      expect(calls).toContainEqual(
        expect.objectContaining({
      ...
      expect(calls).toContainEqual(
        expect.objectContaining({
          file: "git",
          args: ["worktree", "add", "--detach", worktree, headSha],
          options: expect.objectContaining({ cwd: clone }),
        }),
      );
      expect(
        calls.some(({ file, args }) => file === "git" && args[0] === "clone"),
      ).toBe(false);
    } finally {
  ...
  it("prunes a stale Git registration before recreating a retention-cleaned worktree", async () => {
    const root = await mkdtemp(`${tmpdir()}/pr-atlas-retained-worktree-`);
    const calls: Array<{
      file: string;
      args: string[];
      options?: { cwd?: string };
    ...
    try {
      ...
      await service.setRetentionSettings({ analysisDays: 7, worktreeDays: 1 });
      ...
      const prune = calls.findIndex(
        ({ file, args }) =>
          file === "git" && args.join(" ") === "worktree prune",
      );
