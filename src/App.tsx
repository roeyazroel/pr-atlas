...
import {
  Activity, AlertCircle, ArrowDown, ArrowUp, Bot, Check, CircleHelp,
...
type AccountState = { label: string; detail: string; live: boolean; initials: string; avatarLabel: string };
...
const accountFromBootstrap = (result: Pick<BootstrapResult, 'account' | 'warnings'>): AccountState => result.account
  ? { label: `@${result.account.login}`, detail: result.warnings.join(' ') || 'Read-only GitHub CLI session', live: true, initials: initialsFor(result.account.name ?? result.account.login), avatarLabel: `GitHub account ${result.account.name ?? result.account.login}` }
...
function Avatar({ initials, className = '', label }: { initials: string; className?: string; label?: string }) {
...
function App() {
  ...
  const [account, setAccount] = useState<AccountState>({ label: 'Local fixture', detail: 'GitHub CLI not connected in browser preview', live: false, initials: 'PA', avatarLabel: 'PR Atlas demo account' });
  ...
  const providerIsActive = !electronMode || Boolean(selectedProviderStatus?.installed);
  const activeProviderName = !electronMode ? 'Demo runtime' : selectedProviderStatus?.installed ? selectedProviderStatus.displayName : 'No provider available';
  const providerIndicatorLabel = providerIsActive ? activeProviderName : providersLoading ? 'Detecting providers…' : providerError ? 'Provider detection failed' : 'No provider available';
  const providerIndicatorAria = providerIsActive ? `Active provider: ${activeProviderName}` : `Provider status: ${providerIndicatorLabel}`;
  ...
  useEffect(() => {
    ...
    void api.bootstrap().then((result: BootstrapResult) => {
      ...
      setSelectedRepo((current) => hasStoredRepositorySelection && discoveredRepositories.some((repo) => repo.id === current) ? current : discoveredRepositories[0]?.id ?? '');
      setLiveError(result.warnings.length ? result.warnings.join(' ') : null);
    }).catch((error: unknown) => { setAccount({ label: 'GitHub CLI offline', detail: 'Could not load GitHub discovery. Retry after checking GitHub CLI authentication.', live: false, initials: 'GH', avatarLabel: 'GitHub CLI offline' }); setLiveError(error instanceof Error ? error.message : 'Could not load GitHub discovery.'); }).finally(() => setLiveLoading(false));
  ...
  const allRepositories = useMemo(() => electronMode ? liveRepositories : repositories, [electronMode, liveRepositories]);
  ...
  const selectedRepository = allRepositories.find((repo) => repo.id === selectedRepo) ?? (electronMode ? undefined : repositories[0]);
  const isLiveRepository = selectedRepository?.source === 'github';
  const repoPRs = useMemo(() => !selectedRepository ? [] : isLiveRepository ? (livePRs[selectedRepository.id] ?? []) : pullRequests.filter((pr) => pr.repositoryId === selectedRepo), [isLiveRepository, livePRs, selectedRepository, selectedRepo]);

  useEffect(() => {
    if (!api || !selectedRepository || !isLiveRepository || !selectedRepository.fullName || livePRs[selectedRepository.id]) return;
    let cancelled = false;
  ...
  const baseSelectedPR = repoPRs.find((pr) => pr.id === selectedId) ?? repoPRs[0] ?? (electronMode || isLiveRepository ? null : pullRequests[0]);
  ...
  const runLiveAnalysis = async () => {
    ...
    if (result.status === 'ready' && result.document) {
      ...
      setLivePRs((current) => ({ ...current, [selectedPR.repositoryId]: (current[selectedPR.repositoryId] ?? []).map((pr) => pr.id === selectedPR.id ? { ...pr, status, analysisDiagnostic: result.error?.message } : pr) }));
    }
  };
  ...
  const cancelAnalysis = async () => {
    if (analysis?.live && analysis.runId && api && selectedPR) { await api.cancelAnalysis(analysis.runId); setAnalysisStatus('cancelled'); setAnalysisMessage('Analysis cancelled.'); setLivePRs((current) => ({ ...current, [selectedPR.repositoryId]: (current[selectedPR.repositoryId] ?? []).map((pr) => pr.id === selectedPR.id ? { ...pr, status: 'cancelled' } : pr) })); }
    setAnalysis(null);
  };
  ...
  return (
    <div className="app-shell" data-theme={resolvedTheme}>
      <header className="topbar">
        <div className="brand"><div className="brand-mark" aria-hidden="true"><img src="./favicon.png" alt="" /></div><span>PR Atlas</span></div>
        <div className="topbar-context"><span className={`context-dot ${account.live ? 'live' : 'fixture'}`} /> <span>github.com</span><span className="context-separator">/</span><strong>{selectedRepository?.owner ?? '—'}</strong><span className="auth-state" title={account.detail}>{account.label}</span></div>
        <div className="topbar-actions">
          ...
          <div className={`agent-status ${providerIsActive ? 'active' : 'unavailable'}`} aria-label={providerIndicatorAria}><span className="agent-pulse" /><Bot size={15} /> {providerIndicatorLabel}</div>
          <Avatar initials={account.initials} label={account.avatarLabel} />
          {settingsOpen && <div className="popover settings-popover"><div className="popover-title">Workspace settings</div><fieldset className="theme-fieldset"><legend>Theme</legend><div className="theme-options">{themeModes.map(({ value, label }) => <label className="theme-option" key={value}><input type="radio" name="theme-mode" value={value} checked={themeMode === value} onChange={() => chooseTheme(value)} /><span>{label}</span></label>)}</div><p id="theme-description" className="theme-description">System follows your operating-system appearance.</p></fieldset>{electronMode ? <fieldset className="provider-fieldset"><legend>Analysis provider</legend>{providersLoading && <p className="provider-note">Detecting Claude Code, Codex CLI, and Cursor Agent…</p>}{providerError && <p className="provider-note provider-error">{providerError}</p>}{!providersLoading && !providers.length && !providerError && <p className="provider-note">No provider detection result yet.</p>}{providers.map((status) => <label className={`provider-option ${status.installed ? '' : 'unavailable'}`} key={status.provider}><input type="radio" name="analysis-provider" value={status.provider} checked={selectedProvider === status.provider} disabled={!status.installed} onChange={() => chooseProvider(status.provider)} /><span className="provider-option-main"><strong>{status.displayName}</strong><small>{status.executable}</small></span><span className="provider-option-status">{providerStatusLabel(status)}</span></label>)}</fieldset> : <p className="provider-note">Browser demo runtime only; installed provider detection is available in Electron.</p>}{electronMode && selectedProviderStatus?.installed && <label className="model-setting"><span>Model for {selectedProviderStatus.displayName}</span>{selectedProviderStatus.models?.length ? <SelectMenu ariaLabel={`Model for ${selectedProviderStatus.displayName}`} className="model-select-menu" value={selectedModels[selectedProvider] ?? selectedProviderStatus.models[0]?.id ?? ''} options={selectedProviderStatus.models.map((model) => ({ value: model.id, label: model.label }))} onChange={(value) => setSelectedModels((current) => ({ ...current, [selectedProvider]: value }))} /> : <small>The installed tool did not report selectable models; its configured default will be used.</small>}</label>}<label className="prompt-setting"><span>Supplemental collection guidance</span><textarea aria-label="Supplemental collection guidance" value={customPrompt} maxLength={4000} rows={3} placeholder="Example: collect more migration, rollback, or test evidence" onChange={(event) => setCustomPrompt(event.target.value)} /><small>This may guide additional evidence collection, but cannot change the required walkthrough structure.</small></label><div className="settings-row"><span>Data source</span><strong>{electronMode ? account.live ? 'GitHub CLI + local artifacts' : 'GitHub CLI unavailable' : 'Demo fixture (browser)'}</strong></div><div className="settings-row"><span>Provider</span><strong>{activeProviderName}</strong></div><div className="settings-row"><span>Unprocessed PRs</span><strong>Ask to analyze on selection</strong></div><div className="settings-note">{selectedProviderStatus?.installed ? <>Repository context stays local except when an analysis run sends it to {activeProviderName}’s configured model service.</> : 'Select an installed provider to enable analysis.'}</div></div>}
        </div>
      </header>
      ...
      <div className="workspace">
        <aside className="sidebar">
          <div className="repo-picker">
            ...
            <div className="select-wrap"><span className="repo-icon">{selectedRepository?.name.slice(0, 1).toUpperCase() ?? '—'}</span><SelectMenu ariaLabel="Select repository" className="repo-select-menu" value={selectedRepo} options={visibleRepositories.map((repo) => ({ value: repo.id, label: repo.fullName ?? `${repo.owner}/${repo.name}` }))} onChange={setSelectedRepo} placeholder="Choose repository" /></div>
            <div className="repo-meta">{selectedRepository ? <><span>{selectedRepository.source === 'github' ? 'Live GitHub repository' : selectedRepository.private ? 'Private demo repository' : 'Public demo repository'}</span><span>{selectedRepository.openPRs || (isLiveRepository ? (livePRs[selectedRepository.id]?.length ?? '—') : 0)} open</span></> : <><span>GitHub repositories</span><span>—</span></>}</div>
          </div>
        ...
        <main className="content-pane">
          {selectedPR ? <>
            ...
            <div className="content-scroll"><ViewContent view={view} pr={selectedPR} flowType={flowType} setFlowType={setFlowType} activeAnalysis={activeAnalysis} providerName={activeAnalysisProviderName} analysisMessage={analysisMessage} markGroup={markGroup} reviewed={reviewed} cancelAnalysis={() => void cancelAnalysis()} reopenWalkthrough={() => setView('walkthrough')} openHistoricalRun={(runId) => void openHistoricalRun(runId)} openEvidence={openEvidence} />
            </div>
          </> : <section className="empty-detail" aria-labelledby="empty-detail-title"><div className="empty-detail-content"><div className="empty-detail-mark" aria-hidden="true"><img src="./favicon.png" alt="" /></div><div className="eyebrow">{selectedRepository ? 'Repository ready' : 'GitHub discovery'}</div><h2 id="empty-detail-title">{!selectedRepository ? liveLoading ? 'Loading GitHub repositories' : liveError ? 'GitHub repositories unavailable' : 'No GitHub repositories found' : liveLoading ? 'Loading pull requests' : liveError ? 'Pull requests are unavailable' : 'No open pull requests'}</h2><p>{!selectedRepository ? liveLoading ? 'Finding repositories available to the authenticated GitHub CLI account…' : liveError ? liveError : 'GitHub CLI did not return any repositories for this account.' : liveLoading ? <>Checking GitHub for open work in <strong>{selectedRepository.fullName ?? `${selectedRepository.owner}/${selectedRepository.name}`}</strong>…</> : liveError ? 'Refresh the repository to try the read-only GitHub request again.' : <><strong>{selectedRepository.fullName ?? `${selectedRepository.owner}/${selectedRepository.name}`}</strong> is all clear. New pull requests will appear here with their walkthrough status and review evidence.</>}</p>{!liveLoading && <div className="empty-detail-actions"><button className="primary-button" aria-label={!selectedRepository ? 'Retry GitHub discovery' : 'Refresh pull requests'} onClick={refreshLive}><RefreshCw size={14} /> {!selectedRepository ? 'Retry GitHub discovery' : 'Refresh pull requests'}</button></div>}<div className="empty-detail-meta"><ShieldCheck size={14} /><span>Read-only GitHub data</span><span aria-hidden="true">·</span><span>Local analysis artifacts</span></div></div></section>}
        </main>
      </div>
    </div>
  );
...
function ViewContent({ view, pr, flowType, setFlowType, activeAnalysis, providerName, analysisMessage, markGroup, reviewed, cancelAnalysis, reopenWalkthrough, openHistoricalRun, openEvidence }: { view: View; pr: PullRequest; flowType: Flow['type']; setFlowType: (type: Flow['type']) => void; activeAnalysis: { id: string; stage: number; running: boolean; live?: boolean; provider?: AgentProvider } | null; providerName: string; analysisMessage?: string; markGroup: (group: ChangeGroup) => void; reviewed: Record<string, boolean>; cancelAnalysis: () => void; reopenWalkthrough: () => void; openHistoricalRun: (runId: string) => void; openEvidence: (path: string, line?: number) => void; }) {
  ...
  if (view === 'overview') return <Overview pr={pr} />;
...
function FlowsView({ flows, flowType, setFlowType, evidence, groups, threads, openEvidence }: { flows: Flow[]; flowType: Flow['type']; setFlowType: (type: Flow['type']) => void; evidence: Evidence[]; groups: ChangeGroup[]; threads: ReviewThread[]; openEvidence: (path: string, line?: number) => void }) {
  ...
  return <div className="view-section">
    ...
    <div className="flow-toolbar"><div className="search-box"><Search size={14} /><input aria-label="Search flow nodes" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search flow nodes" /><kbd>/</kbd></div><div className="node-filter" aria-label="Graph node filter">{(['all', 'changed', 'context'] as const).map((kind) => <button key={kind} className={`secondary-button ${nodeFilter === kind ? 'active' : ''}`} aria-label={kind === 'all' ? 'All nodes' : kind === 'changed' ? 'Changed nodes' : 'Context nodes'} onClick={() => setNodeFilter(kind)}>{kind === 'all' ? 'All' : kind === 'changed' ? 'Changed' : 'Context'}</button>)}</div><label className="group-highlight"><span>Group</span><SelectMenu ariaLabel="Highlight change group" value={highlightedGroup} options={[{ value: '', label: 'None' }, ...graphGroups.map((group) => ({ value: group.id, label: group.title }))]} onChange={setHighlightedGroup} /></label><button className="secondary-button" aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(MIN_GRAPH_ZOOM, value - 10))}>−</button><button className="secondary-button" aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(160, value + 10))}>+</button><button className="secondary-button" onClick={fitToView}>Fit to view</button><button className="secondary-button" onClick={resetView}>Reset zoom</button><span className="zoom-status" role="status" aria-label="Zoom level">{zoom}%</span></div>
